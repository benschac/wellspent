...
export function validateConfig(input) {
  ...
  if ("captureToken" in input || "token" in input)
...
export function eventFromHook(input, config, now = new Date()) {
  ...
  const digest =
    typeof invocation === "string" && invocation.length
      ? hash(
          `${config.apiOrigin}\0${config.sessionId}\0${thread}\0${input.hook_event_name}\0${invocation}`,
        )
      : null;
  const id = digest
...
function destination(config) {
  return hash(`${config.apiOrigin}\0${config.sessionId}`);
}
...
async function wasResolved(directory, id) {
  ...
  try {
    ...
    if (
      ...
      !validQueuedEvent(rejected.event) ||
...
export async function flush(
  configPath,
  config,
  { token = process.env[TOKEN_ENV], fetchImpl = fetch, maxBatches = 1 } = {},
) {
  if (!token || /[\r\n]/.test(token))
  ...
  if (command === "capture") {
    ...
    process.stdout.write(
      JSON.stringify({
        ...
        tokenAvailable: Boolean(process.env[TOKEN_ENV]),
      }) + "\n",
    );
