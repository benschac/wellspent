...
const config = validateConfig({
  apiOrigin: "https://timer.example",
  sessionId: "c10b8783-e660-4e87-b378-7e754658084c",
});
const hook = {
  ...
  tool_use_id: "call_1",
  tool_name: "Bash",
  tool_input: { command: "cat /Users/alice/private.txt TOKEN=secret" },
  tool_response: { exit_code: 0, output: "private transcript sk-secret" },
...
function acknowledgeAll(_url, options) {
  return Promise.resolve(
    Response.json({
      acceptedEventIds: JSON.parse(options.body).events.map(({ id }) => id),
    }),
  );
}
...
describe("metadata capture", () => {
  test("captures only allowlisted metadata, never raw inputs, output, paths or prompts", () => {
    ...
    expect(JSON.stringify(event)).not.toMatch(
      /alice|private|secret|transcript|TOKEN|command/,
    );
    expect(event.occurredAt).toBe("2026-09-05T12:00:00.000Z");
    expect(
      eventFromHook({ ...hook, tool_response: "exit code 0" }, config).summary,
    ).toBe("Tool completed: Bash.");
  });
  test("turn text requires explicit consent and remains agent-reported", () => {
    const input = {
      ...hook,
      hook_event_name: "Stop",
      last_assistant_message: "Fixed login in /Users/alice/code. token=private",
    };
    expect(eventFromHook(input, config).summary).toBe(
      "Assistant turn completed.",
    );
    const shared = eventFromHook(input, {
      ...config,
      shareAssistantSummary: true,
    });
    expect(shared.summary).toStartWith("Agent-reported: Fixed login");
    expect(shared.summary).not.toMatch(/alice|private/);
    expect(
      eventFromHook(
        { ...input, last_assistant_message: "a".repeat(4000) },
        { ...config, shareAssistantSummary: true },
      ).summary.length,
    ).toBeLessThanOrEqual(2000);
  });
  test("ignores unsupported events and its own capture commands", () => {
    expect(
      eventFromHook({ ...hook, hook_event_name: "UserPromptSubmit" }, config),
    ).toBeNull();
    expect(
      eventFromHook(
        {
          ...hook,
          tool_input: { command: "node /opt/timer-capture.mjs flush" },
        },
        config,
      ),
    ).toBeNull();
    expect(
      eventFromHook(
        { ...hook, tool_name: "mcp__timer__capture_work_events" },
        config,
      ),
    ).toBeNull();
  });
...
describe("durable delivery", () => {
  test("repeated delivery has a stable UUID and preserves original queued time", async () => {
    ...
    const original = eventFromHook(
      hook,
      config,
      new Date("2026-09-05T12:00:00Z"),
    );
    const duplicate = eventFromHook(
      hook,
      config,
      new Date("2026-09-05T12:05:00Z"),
    );
    ...
    await flush(path, config, {
      token: "test-token",
      fetchImpl: async (_url, options) => {
        expect(JSON.parse(options.body).events[0].occurredAt).toBe(
          original.occurredAt,
        );
  ...
  test("duplicate delivery after acknowledgment is skipped without changing its timestamp", async () => {
    ...
    expect(
      (await flush(path, config, { token: "test", fetchImpl: acknowledgeAll }))
        .acknowledged,
    ).toBe(1);
    const repeated = eventFromHook(
      hook,
      config,
      new Date("2026-09-05T12:05:00Z"),
    );
  ...
  test("concurrent delivery and acknowledgment send only the original event", async () => {
    ...
    await enqueue(path, config, original);
    const delivered = [];
    const upload = flush(path, config, {
      token: "test",
        ...
        await Promise.all(
          Array.from({ length: 8 }, () =>
            enqueue(
              path,
              config,
              eventFromHook(hook, config, new Date("2026-09-05T12:05:00Z")),
            ),
          ),
        );
    ...
    await Promise.all([
      ...
      ...Array.from({ length: 8 }, () =>
        enqueue(
          path,
          config,
          eventFromHook(hook, config, new Date("2026-09-05T12:10:00Z")),
        ),
      ),
    ]);
    expect(delivered).toEqual([original]);
    expect(await enqueue(path, config, eventFromHook(hook, config))).toBe(
      false,
    );
    expect((await queueStatus(path, config)).currentSession).toBe(0);
  });
  ...
  test("tampered queue fields are rejected locally before upload", async () => {
    ...
    const [folder] = await readdir(root);
    await writeFile(
      join(root, folder, `${event.id}.json`),
      JSON.stringify({
        apiOrigin: config.apiOrigin,
        sessionId: config.sessionId,
        event: { ...event, rawOutput: "private" },
      }),
      { mode: 0o600 },
    );
    let requests = 0;
  ...
  test("offline or expired credentials retain data and retry the same IDs", async () => {
    ...
    const event = eventFromHook(hook, config);
    await enqueue(path, config, event);
    const offline = await flush(path, config, {
      token: "test-token",
      fetchImpl: async () => {
    ...
    ).toBe("token_expired_or_revoked");
    const result = await flush(path, config, {
      token: "reissued",
      fetchImpl: async (url, options) => {
        expect(url).toBe(
  ...
  test("removes only explicitly acknowledged events", async () => {
    ...
    expect(
      await flush(path, config, {
        token: "test",
        fetchImpl: async () => Response.json({ acceptedEventIds: [first.id] }),
      }),
    ).toEqual({
      acknowledged: 1,
      quarantined: 0,
      reason: "partial_acknowledgment",
    });
    expect((await queueStatus(path, config)).currentSession).toBe(1);
    expect(
      (
  ...
  test("batches at 50 and retains subsequent batches after failed upload", async () => {
    ...
    for (let index = 0; index < 51; index++)
      await enqueue(
        path,
        config,
        eventFromHook({ ...hook, tool_use_id: `call_${index}` }, config),
      );
    let calls = 0;
    const result = await flush(path, config, {
      token: "test",
  ...
  test("mixed accepted and rejected batches preserve rejected originals and flush later valid events", async () => {
      ...
      fetchImpl: async (url, options) => {
        ...
        if (calls++ === 0) {
          ...
          return Response.json({
            acceptedEventIds: events.slice(2).map(({ id }) => id),
            rejectedEvents: [
              { id: events[0].id, reason: "outside_session" },
              { id: events[1].id, reason: "session_limit" },
            ],
          });
        }
        return acknowledgeAll(url, options);
    ...
    expect(calls).toBe(2);
    expect(await queueStatus(path, config)).toMatchObject({
      currentSession: 0,
      currentSessionRejected: 2,
      otherSessionsRejected: 0,
      rejectedByReason: {
        currentSession: { outside_session: 1, session_limit: 1 },
      },
    });
    const root = join(path, "..", "spool");
    const [folder] = await readdir(root);
    for (const id of quarantinedIds) {
      ...
      expect(JSON.parse(await readFile(file, "utf8"))).toMatchObject({
        apiOrigin: config.apiOrigin,
        sessionId: config.sessionId,
        event: originals.get(id),
      });
      expect((await stat(file)).mode & 0o777).toBe(0o600);
      expect(
        await enqueue(path, config, {
          ...originals.get(id),
          occurredAt: "2026-09-05T12:10:00.000Z",
        }),
      ).toBe(false);
    }
    const changed = await configure(path, {
      ...config,
      sessionId: "d10b8783-e660-4e87-b378-7e754658084c",
    });
    expect(await queueStatus(path, changed)).toMatchObject({
      currentSessionRejected: 0,
      otherSessionsRejected: 2,
      rejectedByReason: {
        otherSessions: { outside_session: 1, session_limit: 1 },
      },
    });
  });
  test("unknown, overlapping, duplicate or invalid dispositions retain the entire batch", async () => {
    ...
    const responses = [
      ...
      {
        acceptedEventIds: [first.id],
        rejectedEvents: [{ id: first.id, reason: "outside_session" }],
      },
      {
        acceptedEventIds: [first.id],
        rejectedEvents: [{ id: second.id, reason: "delete_everything" }],
      },
      {
        acceptedEventIds: [first.id],
        rejectedEvents: [{ id: crypto.randomUUID(), reason: "session_limit" }],
      },
      {
        acceptedEventIds: [first.id],
        rejectedEvents: [
          { id: second.id, reason: "future_timestamp" },
          { id: second.id, reason: "id_conflict" },
        ],
      },
  ...
  test("durable quarantine survives interrupted active-file deletion without resending", async () => {
    ...
    const [folder] = await readdir(root);
    await writeFile(
      join(root, folder, `${event.id}.rejected`),
      JSON.stringify({
        apiOrigin: config.apiOrigin,
        sessionId: config.sessionId,
        event,
  ...
  test("switching configuration never retargets previously queued events", async () => {
    ...
    await flush(path, changed, {
      token: "new-session",
      fetchImpl: async (url, options) => {
        expect(url).toContain(changed.sessionId);
        expect(
          JSON.parse(options.body).events.map(({ id }) => id),
  ...
  test("configuration and queue use private permissions and store no credential", async () => {
    ...
    expect(() => validateConfig({ ...config, captureToken: "secret" })).toThrow(
      "Keep the token out",
    );
    expect(() =>
      validateConfig({ ...config, apiOrigin: "http://untrusted.example" }),
    ).toThrow("HTTPS");
  });
});
