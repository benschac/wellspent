 ...
 */
export const profiles = appSchema
  .table(
    "profiles",
    {
...
export const focusSessions = appSchema
  .table(
    "focus_sessions",
    {
    ...
    (table) => [
      index("focus_sessions_user_created_idx").on(
        table.userId,
        table.createdAt,
      ),
      check(
        "focus_sessions_state_check",
        sql`(${table.status} = 'running' and ${table.runningSince} is not null and ${table.completedAt} is null) or (${table.status} = 'paused' and ${table.runningSince} is null and ${table.completedAt} is null) or (${table.status} = 'completed' and ${table.runningSince} is null and ${table.completedAt} is not null)`,
      ),
      check(
        "focus_sessions_counters_check",
        sql`${table.elapsedMs} >= 0 and ${table.revision} >= 1 and ${table.recapRevision} >= 0`,
      ),
    ],
  )
  .enableRLS();
...
export const focusTransitions = appSchema
  .table(
    ...
    (table) => [
      ...
      uniqueIndex("focus_transitions_revision_unique").on(
        table.sessionId,
        table.revision,
      ),
      check(
        "focus_transitions_action_check",
        sql`${table.action} in ('start', 'pause', 'resume', 'finish')`,
      ),
    ],
  )
  .enableRLS();

export const focusWorkEvents = appSchema
  .table(
    "focus_work_events",
    {
      sessionId: uuid("session_id")
    ...
    (table) => [
      ...
      check(
        "focus_work_events_source_check",
        sql`(${table.source} = 'codex' and ${table.kind} in ('tool_completed', 'turn_completed')) or (${table.source} = 'manual' and ${table.kind} = 'note')`,
      ),
    ],
  )
  .enableRLS();

export const focusCaptureTokens = appSchema
  .table("focus_capture_tokens", {
    sessionId: uuid("session_id")
      .primaryKey()
      .references(() => focusSessions.id, { onDelete: "cascade" }),
    tokenHash: text("token_hash").notNull(),
...
export const googleCalendarConnections = appSchema
  .table(
...
export const googleCalendarSubscriptions = appSchema
  .table(
...
export const googleCalendarEventLinks = appSchema
  .table(
...
export const googleCalendarInboundChanges = appSchema
  .table(
...
export const googleCalendarJobs = appSchema
  .table(
    ...
    (table) => [
      ...
      check("google_calendar_jobs_attempts_check", sql`${table.attempts} >= 0`),
      foreignKey({
        columns: [table.connectionId],
        foreignColumns: [googleCalendarConnections.id],
