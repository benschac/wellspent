...
| ----- | -------- | --------------------------- |
| Phase 0: protocol and measurement harness | **Narrow behavior implemented** | Focus Zod contracts, server transition/evidence tests, and browser projection tests exist. Shared command schemas and timing/projection behavior now live in `@repo/session-domain` (2026-09-07), with server/browser lifecycle fixtures. Versioned sync envelopes, repository conformance/fault fixtures, and latency instrumentation remain. |
