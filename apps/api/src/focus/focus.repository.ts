...
function serialize(row: SessionRow): FocusSession {
  return {
    ...
    recapRevision: row.recapRevision,
...
export class FocusRepository {
  ...
  private async lock(
    ...
    const [session] = await tx
      .select()
      .from(focusSessions)
      .where(and(bySession(sessionId), eq(focusSessions.userId, userId)))
      .for("update");
    if (session === undefined)
      throw new ORPCError("NOT_FOUND", { message: "Focus session not found" });
    return session;
  }
  ...
  async create(userId: string, input: CreateFocusInput): Promise<FocusSession> {
    return this.database.transaction(async (tx) => {
      ...
      if (created !== undefined) {
        if (isBefore(at, subHours(now, 7 * 24)))
      ...
      const [command] = await tx
        ...
        .where(
          and(
            eq(focusTransitions.sessionId, input.id),
            eq(focusTransitions.commandId, input.commandId),
          ),
        );
      if (command?.fingerprint !== signature)
        throw new ORPCError("CONFLICT", {
          message: "Session ID or command ID was already used",
        });
  ...
  async transition(
    ...
    return this.database.transaction(async (tx) => {
      ...
      const [duplicate] = await tx
        ...
        .where(
          and(
            eq(focusTransitions.sessionId, input.sessionId),
            eq(focusTransitions.commandId, input.commandId),
          ),
        );
      if (duplicate !== undefined) {
        if (duplicate.fingerprint !== signature)
          throw new ORPCError("CONFLICT", {
      ...
      if (session.revision !== input.expectedRevision)
        throw new ORPCError("CONFLICT", {
          message:
            "The timer changed on another device; refresh before trying again",
        });
      const now = new Date();
      const at = validateEventTime(input.occurredAt, now);
      const [previous] = await tx
        .select()
        .from(focusTransitions)
        .where(eq(focusTransitions.sessionId, session.id))
        .orderBy(desc(focusTransitions.revision))
        .limit(1);
      if (previous === undefined)
        throw new Error("Focus session has no starting transition");
  ...
  private async detail(tx: Transaction, session: SessionRow) {
    const transitions = await tx
      ...
      .from(focusTransitions)
      .where(eq(focusTransitions.sessionId, session.id))
      .orderBy(asc(focusTransitions.revision));
    const rows = await tx
      .select()
      .from(focusWorkEvents)
      .where(eq(focusWorkEvents.sessionId, session.id))
      .orderBy(asc(focusWorkEvents.occurredAt), asc(focusWorkEvents.id));
    const events: WorkEvent[] = rows.map((row) => ({
      id: row.id,
      source: row.source,
  ...
  async get(userId: string, sessionId: string) {
    ...
    return this.database.transaction(async (tx) =>
      this.detail(tx, await this.lock(tx, userId, sessionId)),
    );
  }
  ...
  async updateRecap(userId: string, input: UpdateRecapInput) {
    return this.database.transaction(async (tx) => {
      ...
      if (session.recapRevision !== input.expectedRevision) {
        if (
          session.recapRevision === input.expectedRevision + 1 &&
          session.recapText === input.text
        )
          return this.detail(tx, session);
        throw new ORPCError("CONFLICT", {
          message:
            "The recap changed on another device; reload it before saving",
        });
      }
      const [updated] = await tx
        .update(focusSessions)
        .set({
          recapText: input.text,
          recapRevision: session.recapRevision + 1,
          updatedAt: new Date(),
        })
        .where(bySession(session.id))
        .returning();
      if (updated === undefined)
        throw new Error("Recap update returned no session");
  ...
  private async insertEvents(
    ...
    const existing = await tx
      .select({
        id: focusWorkEvents.id,
        fingerprint: focusWorkEvents.fingerprint,
      })
      .from(focusWorkEvents)
      .where(eq(focusWorkEvents.sessionId, session.id));
    const signatures = new Map(
      existing.map((event) => [event.id, event.fingerprint]),
    );
    ...
    const reject = (id: string, reason: RejectionReason) => {
      if (!partial)
        throw new ORPCError(
          reason === "id_conflict" ? "CONFLICT" : "BAD_REQUEST",
          {
            message:
              reason === "id_conflict"
                ? "Event ID was already used for different evidence"
                : "Evidence is outside the permitted time or session limit",
          },
        );
    ...
    for (const { event, signature } of incoming.values()) {
      ...
      if (
        event.source === "codex" &&
        (isBefore(at, session.createdAt) ||
          (session.completedAt !== null && isAfter(at, session.completedAt)))
      ) {
  ...
  async addNote(userId: string, input: AddFocusNoteInput) {
    return this.database.transaction(async (tx) => {
      const session = await this.lock(tx, userId, input.sessionId);
      await this.insertEvents(tx, session, [
        {
          id: input.id,
          source: "manual",
  ...
  async revokeCaptureToken(userId: string, sessionId: string) {
    return this.database.transaction(async (tx) => {
      await this.lock(tx, userId, sessionId);
      await tx
        .delete(focusCaptureTokens)
        .where(eq(focusCaptureTokens.sessionId, sessionId));
  ...
  async ingest(
    ...
    sessionId: string,
    inputs: WorkEventInput[],
  ) {
    const match = /^Bearer (timer_capture_[A-Za-z0-9_-]{43})$/.exec(
      authorization ?? "",
    );
    if (match?.[1] === undefined)
      throw new ORPCError("UNAUTHORIZED", {
        message: "A valid session capture token is required",
      });
    ...
    return this.database.transaction(async (tx) => {
      ...
      const [credential] = await tx
        ...
        .where(
          and(
            eq(focusCaptureTokens.sessionId, sessionId),
            eq(focusCaptureTokens.tokenHash, hash),
          ),
        );
      if (
        session === undefined ||
        credential === undefined ||
        !isAfter(credential.expiresAt, new Date())
      )
        throw new ORPCError("UNAUTHORIZED", {
          message: "Capture token is invalid, expired, or revoked",
        });
