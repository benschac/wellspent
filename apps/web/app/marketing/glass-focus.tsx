...
export function GlassFocus() {
  ...
  useEffect(() => {
    ...
    void import("./focus-scene").then(({ createFocusScene }) => {
      if (disposed) return;
      let scene: ReturnType<typeof createFocusScene>;
      try { scene = createFocusScene(canvas); } catch { return; }
      ...
      function draw() {
        ...
        if (Math.abs(targetX - currentX) + Math.abs(targetY - currentY) > 0.002 && !reduceMotion.matches) {
      ...
      function motionChange() { stop(); currentX = 0; currentY = 0; targetX = 0; targetY = 0; scene.render(); }
      const resize = new ResizeObserver(() => { scene.resize(host!.clientWidth, host!.clientHeight); });
      resize.observe(host);
      ...
      const contextLost = (event: Event) => { event.preventDefault(); stop(); delete host.dataset.glassReady; };
    ...
    }).catch(() => { /* The accessible DOM control is the WebGL fallback. */ });
    return () => { disposed = true; cleanup?.(); };
  }, []);
  return <canvas ref={canvasRef} className="gh-glass-canvas" aria-hidden="true" data-animation-active="false" />;
}
